...
SpiralProblemInterface::SpiralProblemInterface(
    std::vector<Eigen::Vector2d> points, std::vector<int> sharp_point_idx)
    : init_points_(std::move(points)) {
      // std::cout << "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&create ipopt pro" << std::endl; 
  ...
  for (int i = 0; i + 1 < num_of_points_; ++i) {
    if(ind_ind < sharp_point_idx.size() &&  i == sharp_point_idx[ind_ind]) {
      ++ind_ind;
      std::cout << "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&arrive sharp point" << std::endl; 
      // std::cout << "pos: " << init_points_[i].x() << ", " << init_points_[i].y() << std::endl; 
    }
  ...
  for (int i = 1; i < num_of_points_; ++i) {
    ...
    std::cout << "rela_theta: " <<relative_theta_.back() + theta_diff << std::endl; 
  }
...
SpiralProblemInterface::SpiralProblemInterface(
    ...
    : init_points_(std::move(points)) {
      std::cout << "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&arrive sharp point2" << std::endl; 
  num_of_points_ = static_cast<int>(init_points_.size());
  // CHECK_GT(num_of_points_, 1);
