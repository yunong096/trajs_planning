 ...
 * 
 */
class LineFrontConstraint : public constraints::Constraint<constraints::NegativeOrthant> {
 public:
  ...
  void Evaluate(const VectorXdRef& x, const VectorXdRef& /*u*/,
    ...
    const double ptheta = x(theta_index_);
    for (size_t i = 0; i < line_constraints_.size(); ++i) {
      // store negative since it must be in the negative orthant
      c(i) = (line_constraints_[i](0) * (px + 2.7 * cos(ptheta)) + line_constraints_[i](1) *  (py + 2.7 * sin(ptheta)) - line_constraints_[i](2));
    }
  }
  ...
  void Jacobian(const VectorXdRef& x, const VectorXdRef& /*u*/,
    ...
    for (size_t i = 0; i < line_constraints_.size(); ++i) {
      ...
      jac(i, 2) = -line_constraints_[i](0) * 2.7 * sin(ptheta) +line_constraints_[i](1) * 2.7 * cos(ptheta); //theta系数
    }
  }
