#include <ros/ros.h>
#include <tf/transform_broadcaster.h>
#include <nav_msgs/Odometry.h>
#include <visualization_msgs/Marker.h>
#include <visualization_msgs/MarkerArray.h>
#include <tf/transform_broadcaster.h>

visualization_msgs::Marker car_marker;
ros::Publisher car_publish;
double acc = 0, steer=0, is_stop = -1;
double wheel_base = 2.7; // 轴距（前后轮之车轮轴距离）
double start_x = 0, start_y=0, start_theta=0,start_speed=0;

void visualization_car(const double x, const double y, const double th)
{
    double Length = 4.7; // 车长
    double width = 2;  // 车宽
    car_marker.pose.orientation = tf::createQuaternionMsgFromYaw(th);
    car_marker.header.frame_id = "map";
    car_marker.header.stamp = ros::Time::now();
    car_marker.ns = "basic_shapes";
    car_marker.id = 0; // 注意了
    car_marker.type = visualization_msgs::Marker::ARROW;
    car_marker.action = visualization_msgs::Marker::ADD;
    car_marker.pose.position.x = x;
    car_marker.pose.position.y = y;
    car_marker.pose.position.z = 0;
    car_marker.scale.x = Length;
    car_marker.scale.y = width;
    car_marker.scale.z = 1.0;
    car_marker.color.r = 1.0f;
    car_marker.color.g = 1.0f;
    car_marker.color.b = 0.0f;
    car_marker.color.a = 1.0;
    car_marker.lifetime = ros::Duration();
    car_publish.publish(car_marker);
}

void control_data(const geometry_msgs::Vector3 &v)
{
    // ROS_WARN("sub control state");
    acc = v.x;
    steer = v.y;
    is_stop = v.z;
}

void car_start(const geometry_msgs::Vector3 &msg)
{
    start_x = msg.x;
    start_y = msg.y;
    start_theta = msg.z;
    start_speed = 0.0;

    acc = 0.0;
    steer = 0.0;
}

int main(int argc, char **argv)
{
    ros::init(argc, argv, "car_simulation");
    ros::NodeHandle n;
    tf::TransformBroadcaster odom_broadcaster;
    ros::Publisher odom_pub = n.advertise<nav_msgs::Odometry>("/lyn/odom", 100); //发布位姿
    // ros::Publisher contorl_pub = n.advertise<geometry_msgs::Vector3>("/lyn/control", 10); //发布上一时刻控制量
    car_publish = n.advertise<visualization_msgs::Marker>("/lyn/car/car_state", 100); //可视化车辆当前maker
    ros::Subscriber sub = n.subscribe("/lyn/car/control_car", 100, control_data);           // 订阅控制信息
    ros::Subscriber car_start_pose = n.subscribe("/lyn/car/car_start_pose", 10, car_start); // 订阅起点信息
    ros::Time cur_time, last_time; //用于计时，这里先不使用
    cur_time = ros::Time::now();
    last_time = cur_time;
    ros::Rate rate(10); //频率0.1

    while (n.ok())
    {
        cur_time = ros::Time::now();

        visualization_car(start_x, start_y, start_theta);

       // double dt = (cur_time - last_time).toSec();
        // double  dt = 0.1;
        // double delta_x = start_speed * cos(start_theta) * dt;
        // double delta_y = start_speed * sin(start_theta) * dt;
        // double delta_speed = acc * dt;
        // double delta_theta = (start_speed * tan(steer) / wheel_base) * dt;
        // // std::cout <<  "当前位置: (" << start_x << ", "  << start_y << ", " << start_speed << "),  当前控制量： " << acc  <<  ", " << steer <<std:: endl;
        // // std::cout << "dt:" << dt << "\n";
        // start_x += delta_x;
        // start_y += delta_y;
        // start_theta += delta_theta;
        // start_speed += delta_speed;

        //直接赋值位置
        if(is_stop != -1){
            start_speed = hypot(acc - start_x, steer - start_y);
            start_x = acc;
            start_y = steer;
            start_theta = is_stop;
        }
            
        


        geometry_msgs::Quaternion odom_quat = tf::createQuaternionMsgFromYaw(start_theta);

        geometry_msgs::TransformStamped odom_trans;
        odom_trans.header.stamp = cur_time;
        odom_trans.header.frame_id = "map";
        odom_trans.child_frame_id = "odom";

        // 这里填充里程计的数据
        odom_trans.transform.translation.x = start_x;
        odom_trans.transform.translation.y = start_y;
        odom_trans.transform.translation.z = 0.0;
        odom_trans.transform.rotation = odom_quat;

        // 发送
        odom_broadcaster.sendTransform(odom_trans);

        // 填充里程计数据
        nav_msgs::Odometry odom;
        odom.header.frame_id = "map";
        odom.header.stamp = cur_time;
        // 位置信息
        odom.pose.pose.position.x = start_x;
        odom.pose.pose.position.y = start_y;
        odom.twist.twist.linear.y = 0;
        odom.twist.twist.linear.x = start_speed; //速度能否直接赋值？
        odom.pose.pose.orientation = odom_quat;
        // 发布里程计
        odom_pub.publish(odom);

        last_time = cur_time;
        ros::spinOnce();
        rate.sleep();
    }

    return 0;
}