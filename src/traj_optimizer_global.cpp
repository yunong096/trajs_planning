...
namespace plan_manage {
      ...
      std::vector<std::vector<Eigen::MatrixXd>> &hPoly_container,std::vector<int> singuls,double now, double help_eps)
  {
    ...
    for(int i = 0; i < trajnum; i++){
      ...
      ROS_WARN("init and final state set finished");
    ...
    int result = lbfgs_global::lbfgs_optimize(
        x,
        final_cost,
    ...
    if (result == lbfgs_global::LBFGS_CONVERGENCE ||
        result == lbfgs_global::LBFGS_CANCELED ||
        result == lbfgs_global::LBFGS_STOP||result == lbfgs_global::LBFGSERR_MAXIMUMITERATION)
    {
      flag_force_return = false;
      flag_success = true;

    } 
    else if (result == lbfgs_global::LBFGSERR_MAXIMUMLINESEARCH){
    ...
    {
      ROS_WARN("Solver error. Return = %d, %s. Skip this planning.", result, lbfgs_global::lbfgs_strerror(result));
    }
  ...
  { 
    ...
    for(int trajid = 0; trajid < opt->trajnum; trajid++){
      ...
      smoo_cost = opt->jerkOpt_container[trajid].getTrajJerkCost(1);
...
double PolyTrajOptimizer::debugGradCheck(const int i_dp, // index of constraint point
  ...
  void PolyTrajOptimizer::setParam() {
    ...
    wei_feas_ = 2500; //2500 
    wei_cur_ = 20000; //10000
    wei_d_cur_ = 1000; //1000
    wei_speed_ = 0.0;
    wei_sqrvar_ = 500.0;
    wei_time_ = 0.0; //500.0;
